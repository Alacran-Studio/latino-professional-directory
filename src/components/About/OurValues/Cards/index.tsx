...
export default function Cards({ cards }: Props) {
  ...
  return (
    <div className="mx-auto grid min-h-screen max-w-[1200px] grid-cols-1 place-items-center gap-6 sm:grid-cols-2 md:gap-3">
      {cards.map((card, index) => (
        <div
          key={index}
          className={`${card.borderColor} mx-auto flex flex-col items-center rounded-2xl border-4 p-6 text-center sm:min-h-72 sm:max-w-96 md:min-h-64 md:border-[6px] lg:max-w-[500px]`}
        >
