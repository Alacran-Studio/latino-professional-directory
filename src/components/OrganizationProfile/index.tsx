...
import OrgWebsiteLink from "./OrgWebsiteLink";
import SocialLinks from "./SocialLinks";
import type { OrganizationProfileUI, OrgProfilePhoto } from "@/types/organization";

const COVER_FALLBACK =
  "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1200&h=400&fit=crop";

interface OrganizationProfileProps {
  org: OrganizationProfileUI;
}

export default function OrganizationProfile({ org }: OrganizationProfileProps) {
  const {
    id,
    name,
    description,
      ...
      <div className="-mt-16 relative z-10 mx-4 rounded-xl border border-border bg-card p-6 shadow-lg sm:mx-7 md:mx-14">
        <div className="flex flex-col items-center gap-4 font-lexend sm:flex-row sm:items-start">
          ...
          <div className="text-center sm:text-left">
            <h1 className="text-xl font-bold uppercase tracking-wide sm:text-2xl">{name}</h1>
            {isValidString(short_description) && (
              <p className="mt-1 text-base text-secondary-foreground">{short_description}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 space-y-8 px-4 sm:px-7 md:px-14">
        ...
        {isValidString(description) && (
          <section className="rounded-xl border border-border bg-card p-6 shadow-lg font-lexend">
            <h2 className="mb-4 text-lg font-bold uppercase tracking-wide sm:text-xl">About Us</h2>
            <Paragraph className="text-sm text-secondary-foreground">{description!}</Paragraph>
          </section>
        )}

        {/* Contact Info + Focus Industries */}
        {(isValidString(cityText) || isValidString(website_url) || hasSocialLinks || industries.length > 0) && (
              ...
              <section className="rounded-xl border border-border bg-card p-6 shadow-lg">
                ...
                <div className="space-y-3">
                  ...
                  {isValidString(website_url) && (
                    <OrgWebsiteLink orgId={id} orgName={name} websiteUrl={website_url!} />
                  )}
                  <SocialLinks
        ...
        {gallery_photos.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {gallery_photos.map((photo: OrgProfilePhoto) => (
                <div
                  key={photo.id}
